...
# 0073978
# Comp 541 asignment 3 fall 2022
...
def _calculate_epoch_loss_and_metrics(
    ...
    if display_metrics:
        ...
        hist_metric.append(epoch_metric)
...
class RNN_Language_Network(Pytorch_Seq2Dense):
    def __init__(self, hparams:dict=None):
        ...
        # Permutation may be necessary because RNN output shape and lossd functin input shape are different.
...
class RNNClassifier(ConsonantVowelClassifier):
    ...
    def train_model(self, dataset, verbose:bool=True, saveto:str=None, **kwargs):
        self.history = train_pytorch_model(self.model, dataset, self.model._batchsize, self.model._loss, 
            self.model._optimizer, self.model._optimizerparams, self.model._lossfunctionparams, self.model._learnrate, 
...
def train_rnn_classifier(args, train_cons_exs, train_vowel_exs, dev_cons_exs, dev_vowel_exs, vocab_index):
    ...
    # Train model
    model.train_model(
        [train_dataset, dev_dataset], verbose=True,
...
def train_lm_with_scale(args, train_text, dev_text, vocab_index, scale='small'):
    ...
    # Train model
    model.train_model([train_dataset, dev_dataset], verbose=0,
        # saveto=make_path('assignment3/starter_code/models/'+hparams['model_name']+'.pt'),
        script_before_save=False
        )
